...
| الدفع | Stripe — بطاقة · Apple Pay · Google Pay · الدفع عند الاستلام |
...
npm test           # 55 اختبار تكامل للواجهة البرمجية
...
| GET | `/api/payments/config` | هل الدفع الإلكتروني مفعّل + المفتاح العام |
| POST | `/api/payments/webhook` | مستقبِل أحداث Stripe (موقّع) |
...
المتجر يقبل **البطاقة** و**Apple Pay** و**Google Pay** و**الدفع عند الاستلام**.
الثلاثة الأولى تمر عبر Stripe؛ أبل وجوجل ليستا بوابتَي دفع، بل طريقتان آمنتان
لتمرير بطاقة العميل، فالمبلغ يصل إلى رصيدك في Stripe ثم إلى حسابك البنكي.

### مسار الأموال

```
بطاقة العميل ─▸ Stripe ─▸ رصيدك في Stripe ─▸ حسابك البنكي
    (أو Apple Pay / Google Pay كواجهة للبطاقة نفسها)
```

### الإعداد

1. أنشئ حسابًا على [stripe.com](https://dashboard.stripe.com) وأكمل توثيق النشاط
   (رخصة تجارية وهوية للإمارات) واربط حسابك البنكي لاستلام التحويلات.
2. انسخ المفتاحين من **Developers → API keys** إلى `server/.env`:
...
3. **Apple Pay** يحتاج تسجيل النطاق: **Settings → Payments → Payment methods →
   Apple Pay → Add domain**. جوجل باي لا يحتاج خطوة إضافية.
4. فعّل بطاقات المحافظ من **Settings → Payment methods**.

### الـ webhook

حالة الدفع الحقيقية تصل من Stripe لا من المتصفح، فالمستقبِل إلزامي:
...
# في الإنتاج: Developers → Webhooks → Add endpoint
# https://api.your-domain.com/api/payments/webhook
```

الأحداث المطلوبة: `payment_intent.succeeded` · `payment_intent.payment_failed`
...
| يتطلّب تحقّق 3-D Secure | `4000 0027 6000 3184` |
| مرفوضة | `4000 0000 0000 0002` |

أي تاريخ مستقبلي وأي رمز CVC. لاختبار Apple Pay فعليًا تحتاج Safari على جهاز
Apple مع بطاقة في المحفظة، ونطاقًا مسجّلًا يعمل على HTTPS.

### كيف يتصرّف المتجر

- **بلا مفاتيح Stripe** يبقى المتجر يعمل بالدفع عند الاستلام، ويُعرض خيار
  البطاقة معطّلًا برسالة واضحة، والخادم يرفض `paymentMethod: "card"` بـ 400.
- **المبلغ يُحسب على الخادم دائمًا** من أسعار قاعدة البيانات، ويُرسل إلى Stripe
  بالفلوس. لا يستطيع المتصفح تحديد ما يُدفع.
- **المخزون يُحجز عند إنشاء الطلب** ويُعاد تلقائيًا إن فشل الدفع أو أُلغي أو
  استُرجع — مرة واحدة فقط مهما تكرّر الحدث.
- **إعادة إرسال الأحداث آمنة**: كل معالج مشروط بحالة الدفع الحالية، وحدث فشل
  يصل بعد نجاح لا يُلغي طلبًا مدفوعًا.
- **التوقيع إلزامي**: أي طلب على الـ webhook بلا توقيع صالح يُرفض بـ 400 دون
  أن يمسّ قاعدة البيانات.
- **الإيراد في اللوحة يحتسب المحصّل فقط** — طلب بطاقة لم يُدفع لا يُضاف،
  ويظهر بدلًا عنه في عدّاد «بانتظار الدفع».
...
1. **إشعارات البريد** — لا يُرسل تأكيد طلب بعد.
2. **نموذج التواصل والنشرة البريدية** — يعرضان نجاحًا في الواجهة دون إرسال فعلي.
3. **استعادة كلمة المرور** — غير مطبّقة.
4. **رفع صور المنتجات** — لوحة التحكّم تضبط الشكل واللون فقط.
