...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-card border border-sand-200 bg-white transition hover:-translate-y-1 hover:border-sea-200 hover:shadow-xl hover:shadow-sea-900/5">
      <Link
        href={`/products/${product.slug}`}
        className="relative block bg-sand-50 p-4"
        aria-label={product.name}
      >
        <div className="absolute end-3 top-3 z-10 flex flex-col items-end gap-1.5">
          {product.badge && (
            <span className="rounded-full bg-sea-800 px-2.5 py-1 text-[11px] font-bold text-white">
              {product.badge}
            </span>
          )}
          {off > 0 && (
            <span className="nums rounded-full bg-gold-500 px-2.5 py-1 text-[11px] font-bold text-sea-900">
              -{off}%
            </span>
          )}
        </div>
