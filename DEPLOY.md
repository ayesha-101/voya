...
افتح **SQL Editor** في لوحة مزوّد قاعدة البيانات، الصق محتوى
`voya-setup.sql` كاملًا، واضغط Run. ينشئ الجداول ويملؤها بالتصنيفات
والمنتجات وحسابَي الدخول دفعة واحدة.

استخدم **قاعدة بيانات مخصّصة لهذا المشروع**. إن اضطررت لمشاركة قاعدة
مع تطبيق آخر، أزل التعليق عن سطرَي `CREATE SCHEMA` في أول الملف واضبط
`DB_SCHEMA=voya` في بيئة الخادم — عندها تُعزل جداول المتجر تمامًا عن
`public`.
